...
def _copy_button(text, key):
    """ワンクリックでクリップボードにコピーするボタン（components.html使用）"""
    import streamlit.components.v1 as components
    import html as html_mod
    escaped = html_mod.escape(text).replace("`", "&#96;").replace("$", "&#36;")
    copy_html = f'''
    <textarea id="cptxt_{key}" style="position:fixed;left:-9999px;top:-9999px">{escaped}</textarea>
    ...
    ">📋 コピー</button>
    <script>
    document.getElementById("cpbtn_{key}").addEventListener("click", function() {{
        var ta = document.getElementById("cptxt_{key}");
        ...
        setTimeout(function() {{
            btn.textContent = "📋 コピー";
            btn.style.background = "linear-gradient(135deg,#1DA1F2,#0d8bd9)";
        }}, 1500);
    }});
    </script>
    '''
...
def save_history(mode, input_data, result):
    ...
    entry = {"timestamp": datetime.now(JST).isoformat(), "mode": mode, "input": input_data, "result": result}
...
def load_cached_x_trends(max_age_hours=24):
    ...
    try:
        ...
        # 旧キャッシュ互換: タイムゾーン情報がない場合はJSTとして扱う
        if updated_at.tzinfo is None:
            updated_at = updated_at.replace(tzinfo=JST)
        age_hours = (datetime.now(JST) - updated_at).total_seconds() / 3600
...
def get_cached_x_trends_info():
    ...
    try:
        ...
        age_hours = (datetime.now(JST) - updated_at).total_seconds() / 3600
        # 表示はJSTに変換
        display_time = updated_at.astimezone(JST)
        return {
...
- **読点（、）を減らす。** 1文に読点は最大2つまで。読点の代わりに文を分けるか句点（。）で切る。
  - ❌「石油価格が上がり、物価も上がり、家計が苦しくなる」
  - ✅「石油価格が上がれば物価も上がる。家計が苦しくなるのは当然です」
- **1文は短く。** 1文は40〜60文字を目安にする。長い文は2つに分ける。
- **難しい言葉を使わない。** 専門用語はすぐ後に平易な言い換えを添える。
  - ❌「地政学的リスクが顕在化している」
  - ✅「国と国の位置関係が生むリスクが現実になっている」
- **「つまり」「要するに」を活用。** 複雑な説明の後にひと言でまとめ直す。
- **具体的な例え・スケール感を入れる。** 数字だけでなく身近なものに置き換える。
  - ✅「幅33キロ。東京駅から千葉駅ほどの距離です」
  - ✅「日本の人口が毎年90万人減っている。つまり毎年ほぼ堺市1つ分が消えている計算です」

### 最重要：「前提知識の解説→比較→示唆」の文章構造

すあし社長のポストは「意見表明」ではなく「前提を丁寧に教える→比較で驚きを与える→示唆で考えさせる」の構造が核心です。
読者が「そもそもこれって何？」という疑問を持たないよう、前段でキーワードの意味や背景知識を分かりやすく説明してから本題に入ること。

以下の6段構成を必ず守ってください：

**① 冒頭フック: スクロールを止める1文**（1文）
Xのタイムラインでスクロールの手を止めさせる「衝撃の事実」「意外な数字」「常識を覆す一言」で始める。
良い例: 「幅わずか33キロの海峡が封鎖されただけで、世界の石油価格は一夜にして30%急騰する…」
...
歴史的事例があることで主張の説得力が格段に上がる。

**④ 海外・他国との比較: 視野を世界に広げる**（2-4文）
同じ問題が他の国でどう扱われているかを紹介する。
具体的な国名・人名・制度名・数字を出す。「海外では〜」のような曖昧な表現はNG。
日本と海外の違いを対比させて、構造的な差を浮き彫りにする。
...
3つのポスト案は**固定のパターンではなく、選ばれたトレンドに最適な切り口を考えて決める**こと。
トレンドの内容を分析し、以下の中から最も効果的な3つの方向性を選択する：

- **地理・地政学の切り口** — 地理的な位置関係や地政学的影響を軸に解説
- **歴史の繰り返し切り口** — 過去の類似事例との比較で未来を示唆
- **経済インパクト切り口** — 個人の家計・投資・キャリアへの影響を逆算
- **テクノロジー変革切り口** — 技術革新が構造をどう変えるかを解説
- **構造暴露切り口** — 表面からは見えない仕組みや力学を解き明かす
- **逆転発想切り口** — 常識の逆を突いて新しい視点を提供
- **自分ごと変換切り口** — 遠い話題を読者の日常に結びつける

**重要:** 3案すべてが同じ方向性にならないこと。異なる角度からトレンドを切ることで、最も刺さる案を選べるようにする。

### 事前ファクトチェック（生成時に必ず実行）

ポストを出力する前に、以下のファクトチェックを内部で実施し、**クリアしたもののみ出力すること**：
- 引用した数字・統計は信頼できるソースに基づいているか
- 歴史的事実（年代・人名・事件名）に誤りがないか
- 国名・地名・制度名が正確か
- 因果関係の論理に飛躍がないか
- 不確実な情報には「〜と言われている」「〜の可能性がある」と留保をつけているか
ファクトチェックで不合格の部分がある場合は、修正してから出力する。

### 今回の生成で必ず守ること

1. **冒頭1文は「スクロールを止める衝撃の事実・数字・問い」で始める**
   「幅わずか33キロの海峡が封鎖されただけで、石油価格は30%急騰する」のような意外性ある事実。
   ニュースの要約や「〜してみましょう」からは絶対に始めない。

2. **前提知識を丁寧に解説してから比較に入る**
   読者が「それって何？」と思うキーワードや概念を、先に分かりやすく説明する。
   地理→歴史→構造の順序で積み上げていく。

3. **数字は必ず「比較セット」で使う**
   「A は○％」だけでなく「Aは○％なのに、Bは△％」の対比で驚きを生む。

4. **歴史との比較・海外との比較を必ず入れる**
   具体的な年代・国名・人名・制度名を出す。抽象的な「海外では〜」はNG。
...
■ 現在の日付: {datetime.now(JST).strftime('%Y年%m月%d日')}
...
■ 最重要ルール: シンプルに大きく描く
- 要素は最大3〜4個に絞る。細々した情報を詰め込まない
- 1つの図に1つのメッセージ。「全体の流れ」「構造」「概念」のどれかに集中する
- 文字は超大きいキーワード（2〜4語）と数字（2〜3個）だけ。文章は絶対に入れない
- 余白をたっぷり取る。詰め込みすぎない

■ 図解パターン（内容に合わせて1つ選ぶ）:
- 因果関係 → 大きな太い矢印でA→Bの流れ。要素は2〜3個
- 比較 → 左右に大きく2つ並べる。サイズ差で一目瞭然にする
- 推移 → シンプルな折れ線or階段チャート。ポイントは1〜2箇所だけ強調
- 構造 → 中心に核となる概念、周りに2〜3個の要素を配置

■ 背景:
- 世界地図をベースにした背景（ダークネイビー #1a1a2e 基調）
- トピックに関係する地域をスカイブルー(#1DA1F2)で大きくハイライト
- アクセントにオレンジ(#FFA500)

■ 色とフォーマット:
- キー数字は画像の1/4サイズくらい超大きく太く
- 正方形（1:1）フォーマット
- 右下に小さく「大人の学び直しTV」
"""
...
def _render_post_actions(post_body, key, x_ok):
    """ポストのアクションボタン（コピー・投稿）を表示"""
    col_copy, col_post = st.columns(2)
    with col_copy:
        _copy_button(post_body, f"cp_{key}")
    with col_post:
        if x_ok:
            with st.popover("🐦 投稿", use_container_width=True):
                st.warning("⚠️ Xに投稿します。")
                st.text_area("内容", value=post_body, height=150, key=f"pv_{key}", disabled=True)
                if st.button("✅ 確定して投稿", key=f"cf_{key}", type="primary"):
                    r = post_to_x(post_body)
                    if r["success"]:
...
def display_generated_results(result_text, key_prefix="", auto_fixed=None):
    ...
    # 右カラム用データを事前収集
    fc_results = st.session_state.get(f"{key_prefix.split('_')[0]}_factcheck", {}) if key_prefix else {}
    # トレンドモードの場合は trend_factcheck を参照
    if not fc_results:
        for fc_key_candidate in ["trend_factcheck", "script_factcheck", "image_factcheck"]:
            fc_results = st.session_state.get(fc_key_candidate, {})
            if fc_results:
                break
    ...
    with col_left:
        ...
        for post in posts:
            ...
            with c1:
                if st.button(f"✏️ 修正", key=f"sel_{k}", use_container_width=True):
        ...
        if auto_fixed:
            ...
            for num, fix_data in auto_fixed.items():
                with st.container(border=True):
                    ...
                    body_html = fix_data["fixed"].replace("\n", "<br>")
                    st.markdown(f'<div style="line-height:1.9;font-size:0.98rem;padding:0.5rem 0;">{body_html}</div>', unsafe_allow_html=True)
                    ...
                    with c2:
                        if st.button("✏️ さらに修正", key=f"sel_{fix_k}", use_container_width=True):
                            ...
                            st.rerun()
                    with c3:
                        _render_post_actions(fix_data["fixed"], fix_k, x_ok)
        ...
        if st.session_state.get(revision_key):
            ...
            with c2:
                _render_post_actions(revised_post["body"], k, x_ok)
            with c3:
                if st.button("🗑️ クリア", key=f"clear_rev_{key_prefix}", use_container_width=True):
    ...
    with col_right:
        ...
        has_auto_fix = bool(auto_fixed)
        has_selected = bool(st.session_state.get(selected_key))
        status_items = ["✅ 3案生成済み"]
        if fc_results:
            all_ok = all("✅" in str(v) for v in fc_results.values())
            status_items.append("✅ FC全案OK" if all_ok else "⚠️ FC指摘あり")
        if has_auto_fix:
            status_items.append(f"🔧 {len(auto_fixed)}案を自動修正")
        if has_selected:
            sel = st.session_state[selected_key]
            status_items.append(f"✏️ 案{sel['number']}を選択中")
        if has_revision:
            rev_count = len(st.session_state[revision_key].get("history", [])) + 1
            status_items.append(f"📝 修正{rev_count}回目")
        ...
        if fc_results:
            with st.expander("🔍 ファクトチェック結果", expanded=True):
                for num, fc_text in fc_results.items():
                    st.markdown(f"**案{num}:**")
                    st.markdown(fc_text)
                    if num != list(fc_results.keys())[-1]:
                        st.markdown("---")

        # 自動修正の指摘詳細
        if auto_fixed:
            with st.expander("📋 自動修正の指摘内容"):
                for num, fix_data in auto_fixed.items():
                    st.markdown(f"**案{num}:**")
        ...
        # 修正版のFC結果
        if has_revision:
            fc = st.session_state[revision_key].get("factcheck")
            if fc:
                with st.expander("🔍 修正版FC結果", expanded=True):
                    st.markdown(fc)

        # 修正指示入力
        if has_selected and not has_revision:
            st.markdown("---")
            sel = st.session_state[selected_key]
            sel_title = sel.get('title', f"案{sel['number']}")
            with st.container(border=True):
                st.markdown(f"**✏️ 案{sel['number']}を修正**")
                st.caption(f"{sel_title}（{len(sel['body'])}文字）")
        ...
        # 修正履歴
        if has_revision:
            history = st.session_state[revision_key].get("history", [])
            if history:
