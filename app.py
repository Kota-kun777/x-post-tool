...
### 絶対ルール: 最新情報の検索は生成前に必ず実施

ポストを生成する前に、必ずWeb検索で最新情報を収集し、その情報を参照して事実に基づいた正確な記述にすること。
検索結果がない場合でも、自身の知識の範囲で最新の状況を反映し、不確実な情報には留保をつけること。
...
INFOGRAPHIC_PROMPT = """以下のポスト内容を「概念・構造・流れ」が一目でわかるインフォグラフィック画像を1枚生成してください。
テキストの要約ではなく、ビジュアルで構造や因果関係が理解できる図を目指してください。
...
- 吹き出しで核心を突く一言コメント（「ここが唯一の出口！」「実は逆転する？」等）
- 図の重要部分を指さすポーズで、読者の視線を誘導する
- キャラクターのデザイン（服装・顔・色）は変えないこと

■ レイアウト構成:
1. 画像上部: インパクトのある大タイトル（白or黄色の太字。テーマのキーワードを凝縮）
2. 中央エリア（画像の60%）: 図解の本体 ← ここが最重要。以下のビジュアル要素を組み合わせる:
   - 地図（地理的な話題の場合）: 関係する地域をハイライトし、矢印で影響関係を示す
   - フロー図: 太い矢印で原因→結果の流れ。各ステップにアイコン+大きな数字
   - 比較図: 左右or上下に対比。サイズ差・色差で違いを視覚化
   - 構造図: 中心概念から放射状に要素を配置。線の太さで重要度を表現
   - バーチャート/円グラフ: 数値の比較をグラフィカルに
3. 下部: まとめフロー（3ステップの因果関係を太い矢印でビジュアルに）
   例: [海峡封鎖 ×] →→→ [輸送20% STOP] →→→ [価格30%急騰！]
4. 最下部: テーマ一行 + 右下に小さく「大人の学び直しTV」

■ ビジュアル表現のルール:
- 矢印は太く目立たせる（因果関係・影響の方向が一目でわかる）
- 重要な数字は画像の1/5サイズくらい超大きく（例: "30%UP!" を巨大フォントで）
- ×マーク、チェックマーク、！マーク等の記号で直感的に意味を伝える
- アイコン（船、工場、お金、人物シルエット等）で文字を減らしビジュアルで伝える
- 色の使い分け: 良い/増加=スカイブルー、悪い/危険=オレンジor赤、中立=白/グレー
- 要素間のスペースを適切に取り、視線が自然に流れるレイアウトにする

■ 色とスタイル:
- 背景: ダークネイビー(#1a1a2e)〜ダークブルー(#16213e)
- メイン: スカイブルー(#1DA1F2)
- アクセント: オレンジ(#FFA500)、注意喚起に赤(#FF4444)
- テキスト: 白(#FFFFFF)、強調は黄色(#FFD700)
- 正方形（1:1）フォーマット
"""

INFOGRAPHIC_PROMPT_NO_REF = """以下のポスト内容を「概念・構造・流れ」が一目でわかるインフォグラフィック画像を1枚生成してください。
テキストの要約ではなく、ビジュアルで構造や因果関係が理解できる図を目指してください。
...
def display_generated_results(result_text, key_prefix="", auto_fixed=None):
    """各案ごとにポスト＋FC詳細を横並び表示（自動修正は内部完了済み）"""
    ...
    # ── 自動修正済みの場合はポスト本文を差し替え（内部で完了済みとして表示） ──
    for post in posts:
        if auto_fixed and post["number"] in auto_fixed:
    ...
    # デバッグ
    with st.expander("📄 生成全文（デバッグ用）", expanded=False):
        st.text(result_text)
